    ...
    <div className="max-w-4xl mx-auto px-4 py-8 lg:py-12 font-sans" id="recall-workspace">
      ...
      <div className="flex items-center justify-between border-b border-neutral-100 dark:border-neutral-850 pb-5 mb-8">
        ...
        <span className="text-xs bg-neutral-100 dark:bg-neutral-805 text-neutral-600 dark:text-neutral-400 font-mono border dark:border-neutral-750 px-3 py-1 rounded-full font-medium">
          {mode === "flashcards" ? "Flashcard Leitner Box" : "Syllabus Mastery Quiz"}
        </span>
      </div>
        ...
        <div className="flex flex-col gap-6" id="flashcard-game-flow font-sans">
          {activeCards.length === 0 ? (
            <div className="text-center p-12 border border-dashed border-neutral-200 dark:border-neutral-800 rounded-2xl text-neutral-400">
              No flashcards processed for this session yet.
            </div>
          ) : !completedRecall ? (
            <div className="flex flex-col gap-6 items-center">
              ...
              <div className="w-full flex justify-between items-center max-w-lg mb-2 text-xs font-mono text-neutral-400 dark:text-neutral-500 font-semibold uppercase">
                <span>
                  Flashcard {fcIndex + 1} of {activeCards.length}
                </span>
                ...
                >
                  <Star className={`h-4.5 w-4.5 ${currentFlashcard.reviewState ? "fill-yellow-500" : ""}`} />
                  <span>{currentFlashcard.reviewState ? "Bookmarked" : "Bookmark"}</span>
                </button>
              </div>
                ...
                <AnimatePresence mode="wait">
                  {!isFlipped ? (
                    ...
                    >
                      ...
                      <p className="text-sm lg:text-base font-light tracking-wide text-neutral-250 max-w-md my-auto leading-relaxed">
                        {currentFlashcard.answer}
                      </p>

                      <div className="w-full border-t border-neutral-800 pt-4 flex flex-col gap-2">
                        <span className="text-[10px] font-mono text-neutral-500 font-semibold tracking-wide uppercase">
                          RATE RETENTION LEVEL:
                        </span>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          <button
                            ...
                            className="bg-neutral-805 hover:bg-neutral-750 text-xs py-2 px-2 rounded-lg font-bold border border-neutral-700 transition-colors flex items-center justify-center gap-1 cursor-pointer text-red-400"
                          >
                          ...
                          <button
                            ...
                            className="bg-neutral-805 hover:bg-neutral-750 text-xs py-2 px-2 rounded-lg font-bold border border-neutral-700 transition-colors flex items-center justify-center gap-1 cursor-pointer text-amber-400"
                          >
                          ...
                          <button
                            ...
                            className="bg-neutral-805 hover:bg-neutral-750 text-xs py-2 px-2 rounded-lg font-bold border border-neutral-700 transition-colors flex items-center justify-center gap-1 cursor-pointer text-green-400"
                          >
              ...
              <div className="flex gap-4 mt-6">
                <button
                  ...
                  className={`p-2.5 rounded-lg border flex items-center justify-center cursor-pointer transition-colors ${
                    fcIndex === 0
                      ? "border-neutral-100 text-neutral-300 dark:border-neutral-850 dark:text-neutral-700"
                      : "border-neutral-250 hover:bg-neutral-50 dark:border-neutral-750 dark:hover:bg-neutral-800 text-neutral-700 dark:text-neutral-300"
                  }`}
                >
            ...
            /* Flashcard Completion Dashboard Metrics */
            <motion.div
            ...
            >
              <div className="h-16 w-16 bg-neutral-950 dark:bg-white text-white dark:text-neutral-900 rounded-full flex items-center justify-center font-bold">
                <CheckCircle className="h-8 w-8 text-green-500" />
              </div>

              <div>
                <h3 className="text-2xl font-extrabold tracking-tight text-neutral-950 dark:text-neutral-50 font-sans">
                  Retention Complete!
                </h3>
                <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1 max-w-sm">
                  Congratulations. You have completed the Leitner active recall loop for this lecture.
                </p>
              </div>

              {/* Statistics Breakdown */}
              <div className="w-full border-t border-b border-neutral-100 dark:border-neutral-800/80 py-4 grid grid-cols-3 text-center">
                <div>
                  <span className="text-[10px] font-mono text-neutral-400 uppercase font-semibold block">EASY RATIOS</span>
                  <span className="text-xl font-bold font-mono text-green-500 mt-1 block">
                    {Object.values(fcStats).filter((v) => v === "easy").length}
                  </span>
                </div>
                <div>
                  <span className="text-[10px] font-mono text-neutral-400 uppercase font-semibold block">GOOD METRICS</span>
                  <span className="text-xl font-bold font-mono text-amber-500 mt-1 block">
                    {Object.values(fcStats).filter((v) => v === "good").length}
                  </span>
                </div>
                <div>
                  <span className="text-[10px] font-mono text-neutral-400 uppercase font-semibold block">HARD CHALLENGES</span>
              ...
              <div className="w-full flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  className="flex-1 py-3 text-xs font-bold border border-neutral-300 dark:border-neutral-750 text-neutral-700 dark:text-neutral-300 rounded-lg cursor-pointer hover:bg-neutral-50 dark:hover:bg-neutral-850 flex items-center justify-center gap-1.5 transition-all"
                >
          ...
          {activeQuizzes.length === 0 ? (
            ...
            <div className="flex flex-col gap-6 max-w-xl mx-auto">
              <div className="flex justify-between items-center text-xs font-mono text-neutral-400 uppercase font-semibold mb-2">
                <span>
                  Question {quizIndex + 1} of {activeQuizzes.length}
                </span>
              ...
              <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-6 flex flex-col gap-4">
                <h4 className="text-base lg:text-lg font-bold text-neutral-900 dark:text-neutral-100">
                ...
                <div className="flex flex-col gap-3 mt-2">
                  {currentQuiz.options.map((option, idx) => {
                    ...
                    let btnStyle = "border-neutral-200 dark:border-neutral-800 bg-neutral-50/50 dark:bg-neutral-855 hover:bg-neutral-105 select-none text-neutral-700 dark:text-neutral-300";
                    if (isSelected && !submittedQuizQuestion) {
                      ...
                      btnStyle = "border-green-550 bg-green-500/10 text-green-750 dark:text-green-300 font-bold";
                    } else if (submittedQuizQuestion && isSelected && !isCorrect) {
                      btnStyle = "border-red-550 bg-red-500/10 text-red-750 dark:text-red-300";
                    }
                ...
                {/* Real-time answers text reasoning */}
                  ...
                  <motion.div
                    ...
                    className={`p-4 rounded-lg border text-xs leading-relaxed mt-4 flex flex-col gap-1.5 ${
                      selectedQuizOpt === currentQuiz.correctAnswerIndex
                        ? "bg-green-500/5 border-green-500/15 text-green-850 dark:text-green-300"
                        : "bg-red-500/5 border-red-550/15 text-red-850 dark:text-red-350"
                    }`}
                  >
                    <span className="font-bold">
                      {selectedQuizOpt === currentQuiz.correctAnswerIndex
                        ? "Mastery verified!"
                        : "Concept correction alert"}
                    </span>
                    <p className="opacity-90">{currentQuiz.explanation}</p>
                  </motion.div>
                )}

                {/* Next details button steps */}
                <div className="mt-4 border-t border-neutral-100 dark:border-neutral-800/65 pt-4">
                  {!submittedQuizQuestion ? (
                    <button
                    ...
                    >
                      <span>Check Retention Answer</span>
                    </button>
                  ) : (
                    <button
            ...
            /* Mastery Quiz Completed stats dashboard */
            <motion.div
              initial={{ opacity: 0, scale: 0.98 }}
              animate={{ opacity: 1, scale: 1 }}
              className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-8 text-center flex flex-col items-center gap-6 max-w-lg mx-auto shadow-sm"
            ...
            >
              ...
              <div>
                <h3 className="text-2xl font-extrabold tracking-tight text-neutral-955 dark:text-neutral-50 font-sans">
                  Syllabus Exam Completed!
                </h3>
                <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1 max-w-sm">
                  Comprehensive performance results analyzed using dynamic checkpoints.
                </p>
              </div>
              ...
              <div className="bg-neutral-50 dark:bg-neutral-805 border dark:border-neutral-750 px-8 py-5 rounded-xl flex flex-col items-center justify-center">
                <span className="text-[10px] font-mono text-neutral-450 uppercase font-semibold">RETENTION GRADE</span>
                <span className="text-3xl font-extrabold font-sans text-neutral-900 dark:text-neutral-50 mt-1">
                  {Math.round((quizScore / activeQuizzes.length) * 100)}% ACCURACY
                </span>
                <span className="text-xs text-neutral-400 mt-1 font-mono">
                  {quizScore} out of {activeQuizzes.length} Correct Matches
                </span>
              </div>
